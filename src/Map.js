...
const getMarkerColor = (marker, filter) => {
  ...
  if (filter === "unpaidWaterTax" && marker["Water Tax"] === "Unpaid") {
    ...
    filter === "unpaidPropertyTax" &&
    marker["Property Tax"] === "Unpaid"
  ) {
    return "red";
  } else if (
    filter === "unpaidGarbageTax" &&
    marker["Garbage Tax"] === "Unpaid"
  ) {
    return "red";
    ...
    filter === "unpaidPropertyTax" &&
    marker["Property Tax"] === "Paid"
  ) {
    return "green";
  } else if (
    filter === "unpaidGarbageTax" &&
    marker["Garbage Tax"] === "Paid"
  ) {
    return "green";
  }
};
    ...
    switch (filter) {
      ...
      case "unpaidPropertyTax":
        return markerData.filter(
          (marker) => marker["Property Tax"] === "Unpaid"
        );
      case "unpaidGarbageTax":
        return markerData.filter(
          (marker) => marker["Garbage Tax"] === "Unpaid"
        );
      default:
        return markerData;
    }
